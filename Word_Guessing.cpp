...
// ฟังก์ชันตรวจอักขระ
bool containsInvalidChar(const string &guess);
// หลัก
int main()
{
    mainMenu();
...
void playGame()
{
    ...
    // อธิบาย
    cout << "---------------------------------------------------------------" << endl;
    cout << "Easy: Short words (4-5 letters) and frequently used words" << endl;
    cout << "Medium: Medium-length words (6-8 letters) and common words" << endl;
    cout << "Hard: Words longer than 9 letters and specialized vocabulary" << endl;
    cout << "---------------------------------------------------------------" << endl;
    // เลือกระดับความยาก
    ...
    for (size_t i = 0; i < words.size(); ++i)
    {
        ...
        // ตรวจ
        if (containsInvalidChar(guess))
        {
            cout << "There is an invalid character. Please try again." << endl;
        }
...
bool containsInvalidChar(const string &guess)
{
    ...
    {
        if (!isalnum(ch))
        { // ถ้ามีอักขระที่ไม่ใช่ตัวอักษรหรือตัวเลข
            return true;
        }
    }
    return false;
